...
export function getCategoryForSuppAndCmp_api(company, supplier) {
    return axios.get(apiUrl +"/categoryForSuppAndCmp/"+company+"/"+supplier)
}
...
export function getArticlesForGroupSuppCmp_api(company_id, supplier_id, group_id, page) {
    return axios.get(apiUrl +'/articlesForGroupAndSuppAndCmp/'+company_id+"/"+supplier_id+"/"+group_id+"?page="+page)
}
...
export function searchArticlesForCmp_api(cmp, string, page) {
    return axios.get(apiUrl +"/searchArticlesForCmp/"+cmp+"/"+string+"?page="+page)
}
...
export function getBalance_api(details) {
    return axios.get(apiUrl + "/getBalance/" + details.supplier_id + "/" + details.konto, config)
}
//get result(saldo) - for out of time
export function getBalanceOut_api(details) {
    return axios.get(apiUrl + "/getBalance_out/" + details.supplier_id + "/" + details.konto + "/" + details.date, config)
}
// get analytic card for supplier and 'konto' - get all payment details
export function getAnalyticCard_api(details, page) {
    return axios.get(apiUrl + "/getAnalyticCard/" + details.supplier_id + "/" + details.konto + "?page=" + details.page, config)
}
...
export function getInvoicesForData_api(details) {
    return axios.get(apiUrl + "/getDailyReport/" + details.user + "/" + details.date)
}
...
export function postStoreSurvey_api(details) {
    return axios.post(apiUrl + "/setStoreSurveyHistoryItem", details.payload, config)
}

//post store survey
export function postStoreSurveyAdmin_api(details) {
    return axios.post(apiUrl + "/setStoreSurveyAdminHistoryItem", details.payload, config)
}
...
export function getSSforComercAndData_api(details) {
    return axios.get(apiUrl + "/getStoreSurvey/"+details.payload.user+"/"+details.payload.date);
}
export function getSSforComercAndDataAndCmpAndShop_api(details) {
    return axios.get(apiUrl + "/getStoreSurveyUserDateCmpShop/"+details.payload.user+"/"+details.payload.date+"/"+details.payload.company_id+"/"+details.payload.shop_id);
}
//get schedule history for vendor, date
export function checkScheduleVendorDate_api(details) {
    return axios.get(apiUrl + "/checkSchedule/" + details.payload.operater + "/" + details.payload.date + "/" + details.payload.fullDate);
}
